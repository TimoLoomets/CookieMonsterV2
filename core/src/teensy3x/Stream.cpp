...
#include "teensy3x/Stream.h"
#include "teensy3x/core_pins.h"
#include <math.h>
...
std::string Stream::readString(size_t max)
{
	std::string str;
	size_t length = 0;
	while (length < max) {
...
std::string Stream::readStringUntil(char terminator, size_t max)
{
	std::string str;
	size_t length = 0;
	while (length < max) {
