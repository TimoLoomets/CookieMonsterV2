...
int process_monitor(char *cmd, char *result) {
  ...
  else if (strcasecmp(word, "analogWrite") == 0) {
    char *pin = getNextWord(&place);
    char *state = getNextWord(&place);
    int ipin = strToInt(pin);
    int istate = strToInt(state);
  ...
  else if (strcasecmp(word, "analogRead") == 0) {
    char *pin = getNextWord(&place);
    int ipin = strToInt(pin);
    pinMode(ipin, INPUT);
  ...
  else if (strcasecmp(word, "symbol") == 0) {
    char *name = getNextWord(&place);
    sprintf(send_message, "qSymbol:%s", name);
    strcpy(result, "OK"); 
    return 0;   
  }
  else if (strcasecmp(word, "call") == 0) {
    int args = 0, p[4], ret;
  ...
  else if (strcasecmp(word, "restart") == 0) {
    CPU_RESTART;
    strcpy(result, "");    
    return 0;   
  }
